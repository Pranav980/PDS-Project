...
int i = 0;
while (fgets(line, sizeof(line), file)) {// Initialize values for each row
char state[30];
int year;
...
strcpy(data[i].state, state);data[i].year = year;
...
double predict(double a, double b1, double b2, double b3, double temperature, double rainfall,
double demand) {
...
strcpy(cropColumn, "Pulses_Price");} else if (strcmp(crop, "millets") == 0) {
